...
class CoopPOMDP(object):
    ...
    def calc_a_vector(self, d=1, bs=None, with_a=True):
            ...
            for a_r in range(self.a_r):
                ...
                for a_h in range(self.a_h):
                    for ns, p in self.ns[s][a_r][a_h]:
                        p_a_vector.append(self.a_vector[ns] * self.update[a_r, a_h, s, ns] * p)
        ...
        if with_a:
            self.a_vector_a = {s: {a_r: util.prune(vector, bs) for a_r, vector in vectorA.items()} for s, vectorA in
                               a_vector.items()} if bs is not None else a_vector
        else:
            self.a_vector = {s: util.prune(np.concatenate(list(vector.values()), axis=0), bs) for s, vector in
                             a_vector.items()} if bs is not None else a_vector

    def value_a(self, s, a_h, b):
        return np.max(np.dot(self.a_vector_a[s][a_h], b))
