...
class CoopIRL(object):
    ...
    def _pre_calc(self):
        ...
        for th in range(self.th):
            for s in range(self.s):
                # print(s, th)
                # print(np.unravel_index(s, self.shape), th)
                # print(self.r[:, :, s, th])
                # print(np.max(self.r[:, :, s, th], axis=1))
                # self.sum_r[:, s, th] = util.np.sum(self.o[th, :, s] * self.r[:, :, s, th], axis=1)
    ...
    def calc_a_vector(self, d=1, bs=None, with_a=True):
        if d == 1:
            # self.a_vector = {s: util.unique_for_raw(self.sum_r[:, s, :].copy())
            #                  for s in range(self.s)}
        ...
        for s in range(self.s):
            ...
            for a_r in range(self.a_r):
                ...
                # p_a_vector_nums = []
                print()
                print(np.unravel_index(s, self.shape), a_r)
                for a_h in range(self.a_h):
                    for ns, _p in self.ns[s][a_r][a_h]:
                        # print(ns, _p)
                        # print(self.a_vector[ns])
                        # print(self.t[a_r, a_h, s, ns])
                        ...
                        a_vector_a = np.concatenate([a_vector_a, self.r[a_r, a_h, s] +
                                                     self.a_vector[ns]])
                        # print(np.unravel_index(ns, self.shape), self.a_vector[ns])
                        # print(self.r[a_r, a_h, s])
                print(a_vector_a)
                # print(np.max(a_vector_a, axis=0))
                    # p_a_vector.append(util.unique_for_raw(tmp_p_a_vector))
                    # p_a_vector_nums.append(len(p_a_vector[-1]))

                # print(np.unravel_index(s, self.shape), a_r, a_vector_a)
                # print(np.unravel_index(s, self.shape), a_r, self.a_vector)
                # print(s, a_r)
                # print(p_a_vector)
                # print(p_a_vector.shape)
                # exit()
                ...
                print(a_vector[s][a_r])
                # print(s, a_r, a_vector_a)
    ...
    def value_a(self, s, a_r, b):
        # print()
        # print(b)
        # print(self.a_vector_a[s][a_r])
        # print(np.dot(self.a_vector_a[s][a_r], b))
